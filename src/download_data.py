...
def download_dia(resolution, start_date, end_date):
    ...
    for symbol in df["Symbol"]:
        print(symbol)
        cmd = f'python download_bars.py  --timezone=UTC --size "{resolution}""  --exchange ARCA --start-date {start_date} --end-date {end_date} {symbol}'
        print(cmd)
...
def download_index_future(resolution, start_date, end_date):
    config = [
        {"symbol": "ES", "secType": "CONTFUT", "exchange": "CME"},
        {"symbol": "MES", "secType": "CONTFUT", "exchange": "CME"},
        {"symbol": "NQ", "secType": "CONTFUT", "exchange": "CME"},
        {"symbol": "MNQ", "secType": "CONTFUT", "exchange": "CME"},
    ]

    for c in config:
        cmd = f'python download_bars.py --size "{resolution}"  --exchange {c["exchange"]} --security-type {c["secType"]} --start-date {start_date} --end-date {end_date} {c["symbol"]}'
        print(cmd)
        exit_code = os.system(cmd)
        ...
        if exit_code == 0:
            ...
            print(
                f"Command failed with exit code {exit_code} for symbol {c['symbol']}."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--resolution", type=str, help="Data resolution", default="1 day"
    )
    parser.add_argument("--start_date", type=str, help="Start date", default="20230101")
    parser.add_argument("--end_date", type=str, help="End resoldateution", default="20231231")

    # Parse the command-line arguments
    args = parser.parse_args()
    print(args)
    if args.symbol == "IDX_FUT":
        download_index_future(args.resolution, args.start_date, args.end_date)
    elif args.symbol != "":
        os.system(
            f'python download_bars.py --timezone=UTC --size "{args.resolution}" --exchange ARCA --start-date {args.start_date} --end-date {args.end_date} {args.symbol}'
        )
    elif args.index == "QQQ":
